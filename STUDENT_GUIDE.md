...
`scripts/motor_driver.py` 파일의 9번째 줄 `linear_speed = 0.15` 값을 `0.20`으로 변경해보세요.

### 퀴즈 2. 후진할 때 선속도(linear.x)는 왜 음수(-)일까요?
ROS 표준 좌표계(REP-103)에서 x축 앞쪽이 양수(+), 뒤쪽이 음수(-)이기 때문입니다.

### 도전 과제. 대각선 주행 키 만들어보기
앞으로 가면서 동시에 좌회전하도록 만들려면:
`twist.linear.x = linear_speed`와 `twist.angular.z = angular_speed`를 동시에 주면 곡선으로 회전합니다.
