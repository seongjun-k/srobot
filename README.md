# srobot (TurtleBot Teleop & Direct Motor Control)

> **경량 터틀봇 텔레옵 및 OpenCR 다이나믹셀 직접 제어 ROS 1 Noetic 패키지**  
> 복잡한 전체 브링업(LiDAR, 카메라 등) 없이, **단 하나의 런치 명령**으로 터틀봇 모터를 구동하고 원격 노트북에서 실시간 키보드로 주행을 제어합니다.  
> 
> 학생 교육 및 수업용 실습 교재는 **[학생용 실습 가이드](STUDENT_GUIDE.md)**를 참고하세요.
...
2. **엔터(Enter) 없는 즉각적인 실시간 키보드 입력 (`teleop.launch`)**
   - Linux Raw Terminal(termios)을 통해 키를 누르는 즉시 터틀봇이 반응합니다.
3. **주행 중 실시간 속도 조절 지원**
   - 터틀봇 주행 중에도 정지할 필요 없이 `e` / `c` 키로 실시간 속도를 즉시 증감할 수 있습니다.
4. **네트워크 자동 감지 스크립트 (`run_teleop.sh`)**
   - 노트북의 현재 IP를 자동 감지하여 `ROS_IP`를 할당하므로, Wi-Fi 환경이 바뀌어도 `.bashrc`를 수정할 필요가 없습니다.
5. **초보자 및 교육 친화적 코드**
   - 터틀봇 교육 실습에 맞춰 복잡한 시스템 코드를 배제하고, 학생들이 직접 타이핑하고 이해하기 쉬운 구조로 제작되었습니다.
...
flowchart LR
    ...
    subgraph Robot ["터틀봇 (TurtleBot / Raspberry Pi)"]
...
| `/cmd_vel` | `geometry_msgs/Twist` | `motor_driver.py` | `opencr_core` | 터틀봇 모터 속도 지휘값 (선속도/각속도) |

---

## 사전 준비 (Prerequisites)

터틀봇(라즈베리파이)에서 OpenCR 보드와 시리얼 통신을 위해 `rosserial` 패키지 설치 및 시리얼 포트 권한이 필요합니다.
...
노트북과 터틀봇의 catkin 워크스페이스에 각각 다운로드하여 빌드합니다.
...
> **실시간 속도 조절:** 터틀봇 주행 중에도 `e`, `c` 키를 누르면 멈추지 않고 즉시 속도가 변경됩니다.

---

## 부팅 시 자동 실행 (Systemd Auto-Start)

터틀봇에 매번 SSH로 접속하여 명령어를 치는 번거로움을 없애려면 터틀봇에 systemd 서비스를 등록할 수 있습니다.
...
서비스 활성화 시 터틀봇 전원만 켜면 백그라운드에서 자동 대기하므로, 노트북에서 `./run_teleop.sh <터틀봇IP>`만 켜면 즉시 주행할 수 있습니다.
...
* **해결:** 터틀봇에서 `sudo apt install ros-noetic-rosserial-python ros-noetic-rosserial-msgs`를 실행합니다.

### Q2. `RLException: ERROR: unable to contact ROS master` 에러
* **해결 점검:**
  1. 터틀봇과 노트북이 동일한 Wi-Fi에 연결되어 있는지 확인 (`ping <터틀봇IP>`)
  2. 터틀봇에서 `robot.launch` 또는 roscore가 실행 중인지 확인
